  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6">
      ...
      <div className="animate-in text-center mb-8">
        ...
        <p className="text-muted-foreground">Bérbeadó bejelentkezés</p>
      </div>

      <form onSubmit={handleSubmit} className="w-full max-w-sm space-y-4 animate-in-delay-1">
        <div>
          <label className="text-sm text-muted-foreground block mb-1">Felhasználónév</label>
          <Input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="admin" autoFocus />
        </div>
        <div>
          <label className="text-sm text-muted-foreground block mb-1">Jelszó</label>
      ...
      <p className="text-muted-foreground text-xs mt-8 animate-in-delay-2">
        <Link to="/tenant/login" className="text-primary hover:underline">← Bérlő belépés</Link>
      </p>
    </div>
  );
};
